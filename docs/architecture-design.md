...
所有 submodule 都视为第三方只读依赖。不要直接改 submodule；项目内适配和修复应写在 `providers/open_source/` 或 Research OS 自己的 domain 层。默认运行不得依赖 `research-os` 以外的隐式目录；底层 CLI、公共 skill、submodule 和通过环境变量显式配置的数据缓存除外。
