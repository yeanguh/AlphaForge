...
Research OS 是一个本地优先的 AI 投研循环系统。它的目标不是回答“明天买什么”，而是围绕公开信息、产业链研究、标的验证、买点分析和复盘反馈，帮助人持续训练某个领域的投资判断力。
...
Codex / Claude Code 是执行 Agent。Research OS 不自研 Agent runtime。
...
| `providers/open_source/` | 开源项目和 submodule 的只读 adapter，把外部能力转成 Research OS 统一 packet。 |
...
| `external/` | 非 skill 型开源 submodule。只能通过 `providers/open_source/*` 使用。 |
...
| `TradingAgents-astock` | `external/TradingAgents-astock` | 已通过 `review_packet` 轻量接入评审。 | A 股投委会式第二意见，在 Research OS 已有证据和候选后评审。 |
| `Vibe-Research` | `external/Vibe-Research` | 当前 smoke/reference only。 | 研究工作台、证据组织、报告 workflow 和覆盖度检查参考。 |
| `Vibe-Trading` | `external/Vibe-Trading` | 当前 smoke/reference only。 | 技术面小组、基本面严谨性检查、假设注册、回测/影子账户建议。 |
...
所有 submodule 都视为第三方只读依赖。不要直接改 submodule；项目内适配和修复应写在 `providers/open_source/` 或 Research OS 自己的 domain 层。默认运行不得依赖 `research-os` 以外的本地目录；底层 CLI、公共 skill、submodule 和显式环境变量配置的数据缓存除外。
...
Research OS 应该把能力做成可组合阶段，而不是松散脚本调用。

| 阶段 | 职责 | 主要工具 |
| --- | --- | --- |
| `news-scanner` | 收集公开新闻、研报标题、资讯源条目。 | `investment-news`、公开 feed/search、未来 wechat-feeds |
| `article-reader` | 把文章/标题转成 claim、主题、实体、证据缺口。 | Research OS reader、未来 Vibe-Research workflow adapter |
...
可借鉴的编排骨架是：候选实体合并、证据交叉验证、维度评分、core/watch/reject 分桶、missing evidence 显式化、投委会复核。Research OS 应吸收这个模式，但入口应从“选股池”升级为“产业链研究”。该模式只作为方法论，不构成对其他仓库代码或目录的运行依赖。

## 7. Packet 和 Schema 契约

Research OS 内部阶段之间传递 packet，而不是传递第三方对象或任意 dict。所有 packet 都应具备：
...
| `ReviewPacket` | Research OS | TradingAgents / Codex / Claude | 只读评审输入，不包含 state 写权限。 |
...
- adapter 可以接收第三方格式，但输出必须转成 Research OS packet。
...
Research OS 的长期价值来自论点库，而不只是每日报告。`memory/` 应逐步承载人的投资认知。
...
外部 Agent 项目不能拥有 Research OS 的状态写权限。它们只接收 packet，返回只读 review result。
...
当前状态：submodule + smoke/reference only。
...
它应该在 Research OS 已经选出候选公司后进入，而不是在原始数据扫描阶段进入。
...
它应提升报告工作流和证据组织，不替代 Research OS 核心 loop。
...
Research OS 默认本地优先，但仍会使用 API key、CLI、缓存和第三方数据源。
...
- `Vibe-Research` 和 `Vibe-Trading` 当前还没有进入 runtime research provider；
- 技术面和估值分析已有雏形，但价格结构、箱体、趋势、支撑阻力、目标/失效位仍需加强；
- 报告质量门禁已有，但正式研报稳定对标高质量公开投研文章仍需继续打磨。

明确不做：

- 不隐式依赖 `research-os` 以外的本地仓库、目录或旧工程结构；
...
- 只有在 packet 契约稳定后，才把 Vibe-Trading / Vibe-Research 从 smoke-only 升级为只读 runtime adapter。
